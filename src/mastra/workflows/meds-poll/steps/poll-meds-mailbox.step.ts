...
export const pollMedsMailbox = createStep({
    ...
    execute: async ({ mastra, inputData: { dryRun } }) => {
        ...
        if (!rules) {
            ...
            logger.warn('[poll-meds-mailbox] "meds" todavia no tiene reglas activas, salteo la corrida')
        ...
        for (const mail of mails) {
            try {
                ...
                if (dryRun) {
                    ...
                    logger.info(`[poll-meds-mailbox] (dry-run) ${mail.id} -> "${label}"${isDefault ? ' (default: iría a review)' : ''}`, { year: mail.year, month: mail.month, data })
                    continue
                }
                ...
                if (!result.ok) logger.error(`[poll-meds-mailbox] ${mail.id} clasificado como "${label}" pero el handler falló: ${result.reason}`)
